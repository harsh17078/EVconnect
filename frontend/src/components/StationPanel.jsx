...
export default function StationPanel({ station, onClose, onStartCharge, onReserve, isCharging }) {
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 z-[1200] animate-slide-up">
      <div className="glass rounded-t-3xl p-6 max-h-[75vh] overflow-y-auto shadow-[0_-12px_40px_rgba(0,0,0,.5)]">
        ...
        <div className="grid grid-cols-4 gap-2 mb-5">
          {[
            ...
            { icon: Clock,            label: 'Wait',    value: station.waitMin > 0 ? `${station.waitMin} min` : 'None', color: station.waitMin > 0 ? 'text-amber-400' : 'text-emerald-400' },
            { icon: station.latency > 0 ? Wifi : WifiOff, label: 'Latency', value: station.latency > 0 ? `${station.latency}ms` : 'Down', color: station.latency > 0 ? 'text-white' : 'text-red-400' },
          ...
          <div className="mb-5 bg-white/[.02] border border-white/[.05] rounded-xl p-4">
            <div className="flex justify-between items-center text-xs mb-3">
              ...
              <span className="text-slate-500 text-[10px]">
                Queue: {station.queue} car{station.queue !== 1 ? 's' : ''}
              </span>
            </div>
                ...
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full rounded-sm transition-all relative" style={{ height: `${Math.max(pct, 4)}%` }}>
                    <div className={`absolute inset-0 rounded-sm ${
                      pct > 75 ? 'bg-rose-500/50' : pct > 40 ? 'bg-amber-500/50' : 'bg-emerald-500/50'
                    }`} />
                  </div>
            ...
            {peakHour >= 0 && (
              <p className="text-[10px] text-slate-500 mt-2">
                ⚠️ Peak congestion expected in {peakHour + 1}h ({Math.max(...station.forecast)}% utilization)
              </p>
            )}
          </div>
        )}
          ...
          <div className="mb-5 bg-rose-500/[.06] border border-rose-500/15 rounded-xl p-3 flex items-start gap-3">
            ...
            <div>
              ...
              <div className="text-[10px] text-rose-400/70 mt-0.5">
                Isolation Forest model detected anomalous temp ({station.temp}°C) and voltage ({station.voltage}V) patterns. Maintenance recommended.
              </div>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-3">
            ...
            <>
              <button
                ...
                className="flex-1 h-11 rounded-xl font-bold text-sm flex items-center justify-center gap-2
                  ...
                  hover:from-sky-400 hover:to-cyan-300 disabled:opacity-40
                  shadow-lg shadow-sky-500/20 transition-all active:scale-[.98]"
              >
                <Zap className="w-4 h-4" /> Start Charging
              </button>
              <button
                onClick={() => onReserve(station)}
                className="h-11 px-5 rounded-xl font-semibold text-sm glass border-white/10
                  text-slate-200 hover:bg-white/[.06] transition-all"
              >
                Reserve
              </button>
            </>
          ...
          {station.status === 'occupied' && (
            <div className="flex-1 glass rounded-xl p-3 text-center text-sm text-slate-400">
              🔒 Currently occupied — estimated {station.waitMin} min wait
            </div>
          )}
          {station.status === 'offline' && (
            <button
          ...
          {station.status === 'reserved' && (
            <div className="flex-1 glass rounded-xl p-3 text-center text-sm text-amber-400/80">
              🕒 Reserved — next slot in {station.waitMin} min
            </div>
          )}
        </div>
      </div>
